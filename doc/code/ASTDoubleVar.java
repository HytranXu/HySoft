...
public class ASTDoubleVar extends SimpleNode 
{
	//当赋值时采用表达式或调用函数时需要这些完成后再处理
	private boolean bWaitForChild = false;
	
	//构造函数
	public ASTDoubleVar(int id) 
	{
		super(id);
	}

	public ASTDoubleVar(HySoft p, int id) 
	{
		super(p, id);
	}
	...
	{
		logShow.AppendShow("<------DoubleVar interpret------>",InforType.Infor);
			...
			if(names.length > 1)
			{
					...
					if(null != values)
					{
						for(String obj : values)
						{
							if(null != obj && !obj.isEmpty())
								iniValues.add(Double.valueOf(obj));
							else
								iniValues.add(new Double(0.0));
						}
					}
				}
				//需要计算的
				else
				{
					...
					if(var.bWaitForChild)
					{
						...
						infor.append(key);
						infor.append(" type: double value: ");
						infor.append(value);
				...
				result = tab.setParamArray(names[0],"double",PrepareArray(indexs,iniValues),false,true);
			}
			//单个
			else
			{
				if(value != null && !value.isEmpty())
					result = tab.setParam(key,"double",Double.valueOf(value),true);
				else
				{
					Double bValue = null;
					if(var.bWaitForChild)
					{
						...
						//取具体值
						if(null != objv)
							bValue = getDouble(objv);
					}
					
					if(null != bValue)
						result = tab.setParam(key,"double",bValue,true);
					else 
						result = tab.setParam(key,"double",new Double(0.0),true);
				}				
			}
	...
	private void interpretForCompile()
	{
		...
		for(String key : Arraymap.keySet())
		{
			...
			if(names.length > 1)
			{
				...
				result = tab.addDataDefArray(dataName,names[0],"double",PrepareArray(indexs,iniValues),false);
			}
			//单个
			else
			{
				if(value != null && !value.isEmpty())
					result = tab.addDataDef(dataName,key,"double",Double.valueOf(value));
				else
					result = tab.addDataDef(dataName,key,"double",new Double(0.0));
			}
			...
			if(result == DataTAB.dt_OK)
			{
				...
				infor.append(" mem--> type: double name: ");
				infor.append(key);
				infor.append(" value: ");
				infor.append(value);
				logShow.AppendShow(infor.toString(),InforType.Debug);
