...
public class ASTVecVar extends SimpleNode 
{
	private boolean bWaitForChild = false;
	
	public ASTVecVar(int id) 
	{
		super(id);
	}

	public ASTVecVar(HySoft p, int id) 
	{
		super(p, id);
	}
	...
	{
		logShow.AppendShow("<------VecVar interpret------>",InforType.Infor);
		...
		for(NamesForVar var : VariableNames)
		{
			...
			//vector本身即是数组，不再提供vector[]数组
			if(names.length > 1)
			{
				result = DataTAB.dt_ParamWrong;
			}
			//单个
			else
			{
				if(value != null && !value.isEmpty())
					result = tab.setParam(key,"vector",Integer.valueOf(value),true);
				else
				{
					Integer bValue = null;
