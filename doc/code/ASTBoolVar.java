...
public class ASTBoolVar extends SimpleNode 
{
	//当赋值时采用表达式或调用函数时需要这些完成后再处理
	private boolean bWaitForChild = false;

	//构造函数
	public ASTBoolVar(int id) 
	{
		super(id);
	}

	public ASTBoolVar(HySoft p, int id) 
	{
		super(p, id);
	}
	//执行解释
	...
	{
		logShow.AppendShow("<------BoolVar interpret------>",InforType.Infor);
		...
		for(NamesForVar var : VariableNames)
		{
			...
			if(names.length > 1)
			{
					...
					if(null != values)
					{
						for(String obj : values)
						{
							if(null != obj && !obj.isEmpty())
								iniValues.add(Boolean.valueOf(obj));
							else
								iniValues.add(new Boolean(false));
						}
					}
				}
				//需要计算的
				...
				result = tab.setParamArray(names[0],"bool",PrepareArray(indexs,iniValues),false,true);
			}
			//单个
			else
			{
				if(value != null && !value.isEmpty())
					result = tab.setParam(key,"bool",Boolean.valueOf(value),true);
				else
				{
					Boolean bValue = null;
					if(var.bWaitForChild)
					...
					if(null != bValue)
						result = tab.setParam(key,"bool",bValue,true);
					else 
						result = tab.setParam(key,"bool",new Boolean(false),true);
				}
			}
	...
	private void interpretForCompile()
	{
		...
		for(String key : Arraymap.keySet())
		{
			...
			if(names.length > 1)
			{
				...
				result = tab.addDataDefArray(dataName,names[0],"bool",PrepareArray(indexs,iniValues),false);
			}
			//单个
			else
			{
				if(value != null && !value.isEmpty())
					result = tab.addDataDef(dataName,key,"bool",Boolean.valueOf(value));
				else
					result = tab.addDataDef(dataName,key,"bool",new Boolean(false));
			}
			...
			if(result == DataTAB.dt_OK)
			{
				...
				infor.append(" mem--> type: bool name: ");
				infor.append(key);
				infor.append(" value: ");
				infor.append(value);
				logShow.AppendShow(infor.toString(),InforType.Debug);
