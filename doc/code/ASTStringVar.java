...
public class ASTStringVar extends SimpleNode 
{
	private boolean bWaitForChild = false;
	
	//构造函数
	public ASTStringVar(int id) 
	{
		super(id);
	}

	public ASTStringVar(HySoft p, int id) 
	{
		super(p, id);
	}
	...
	{
		logShow.AppendShow("<------StringVar interpret------>",InforType.Infor);
			...
			if(names.length > 1)
			{
				...
				else
				{
					...
					if(var.bWaitForChild)
					{
						...
						infor.append(key);
						infor.append(" type: string value: ");
						infor.append(value);
				...
				for(; t<MaxIndex; t++)
					iniValues.add(new String(""));
				
				result = tab.setParamArray(names[0],"string",PrepareArray(indexs,iniValues),false,true);
			}
			//单个
			else
			{
				if(value != null && !value.isEmpty())
					result = tab.setParam(key,"string",getStr(value),true);
				else
				{
					String bValue = null;
					if(var.bWaitForChild)
					...
					if(null != bValue)
						result = tab.setParam(key,"string",bValue,true);
					else 
						result = tab.setParam(key,"string",new String(""),true);
				}
	...
	private void interpretForCompile()
	{
		...
		for(String key : Arraymap.keySet())
		{
			...
			if(names.length > 1)
			{
				...
				result = tab.addDataDefArray(dataName,names[0],"string",PrepareArray(indexs,iniValues),false);
			}
			//单个
			else
				result = tab.addDataDef(dataName,key,"string",value);
			...
			if(result == DataTAB.dt_OK)
			{
				...
				infor.append(" mem--> type: string name: ");
				infor.append(key);
				infor.append(" value: ");
				infor.append(value);
				logShow.AppendShow(infor.toString(),InforType.Debug);
